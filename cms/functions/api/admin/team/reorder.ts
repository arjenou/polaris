...
/** Persists the new drag-and-drop display order for one locale's team
 * members. Expects the full ordered list of ids for that locale. */
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const statements = input.orderedIds.map((id, index) =>
    env.DB.prepare("UPDATE team_members SET sort_order = ? WHERE id = ? AND locale = ?").bind(index, id, input.locale),
  );
